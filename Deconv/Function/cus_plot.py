...
def plot_different_figs(args, plot_dict):
    ...
    cur_row += 1
    if 'deconv_cam' in plot_dict.keys():
        for index, cam in enumerate(plot_dict['deconv_cam']):
            fig.add_subplot(len(plot_dict['deconv_cam']), cols, index * cols + cur_row + 1)
            plt.imshow(plot_dict['deconv_cam'][index].data.numpy())
            plt.axis('off')

    plt.savefig(args.test_cam_path, bbox_inches='tight', pad_inches=0)
    plt.close()
