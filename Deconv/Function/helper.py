...
def get_img_path_by_id(args, img_id, dataset):
    raw_imgs = []

    cur_csv = dataset.cur_csv
    target_path = cur_csv[cur_csv['id'].isin(img_id)]['path']
    for path in target_path:
        raw_imgs.append(args.data_root_path + path)

    return raw_imgs


def get_seg_by_path(args, data_dict):
    if 'img_path' in data_dict.keys():
        for index,path in enumerate(data_dict['img_path']):
            plt.imshow(data_dict['raw_imgs'][index])
            plt.savefig('../Save/imgs/test_seg_raw.png', bbox_inches='tight', pad_inches=0)

            input_image = ndimage.imread(path, flatten=False, mode=None)
            for k in [100,200,300,400,500]:
                seg_output = segment(in_image=input_image, sigma=0.5, k=k, min_size=50)/255

                plt.imshow(seg_output)
                plt.savefig('../Save/imgs/test_seg_k_{}.png'.format(k), bbox_inches='tight', pad_inches=0)
