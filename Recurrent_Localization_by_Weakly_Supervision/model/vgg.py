...
class VGG(nn.Module):
    ...
    def forward(self, x, label=None):
        ...
        for i in range(5):
            cam = self.cls(avg_pool)  # （16,200,28,28)
            ...
            # tmp = output_grad.cpu().data.numpy()[0, label.data[0], :, :]
            # plt.imshow(tmp)
            # plt.show()
            ...
            x_gradient_norm = self.normalize_atten_maps(x.grad)
            tmp=torch.ge(x_gradient_norm,0.5).type(torch.FloatTensor).cuda()
            changed_img=x*tmp
            plt.imshow(x.cpu().data.numpy()[0].transpose(1,2,0))
            plt.show()
            plt.imshow(changed_img.cpu().data.numpy()[0].transpose(1,2,0))
            plt.show()
            print()
    ...
    def get_gradient_of(self, var, label):
        ...
        # pos_low = torch.lt(select_var, 0.05)

        # output_grad[pos_low.data] = 1.0
...
def get_vgg(pretrained=False, **kwargs):
...
if __name__ == '__main__':
    ...
    back = final.new_empty(final.size())
    final.backward(gradient=back, retain_graph=True)
    print()
