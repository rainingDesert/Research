...
@click.command()
@click.option('--data_dir', type=click.Path(exists=True), default='../../../Data/push/push_train',
              help='Directory containing data.')
@click.option('--out_dir', type=click.Path(), default='out_dir',
              help='Output directory of the converted data.')
...
@click.option('--create_img_prediction', type=click.INT, default=1,
              help='Create the bitmap image used in the prediction phase')
def main(data_dir, out_dir, sequence_length, image_original_width, image_original_height, image_original_channel,
         image_resize_width, image_resize_height, state_action_dimension, create_img, create_img_prediction):
    ...
    with tf.Session() as sess:
        ...
        for i in range(sequence_length):
            ...
            if (create_img_prediction == 1):
                ...
                image_pred = tf.reshape(image_pred,
                                        [1, image_original_height, image_original_width, image_original_channel])
        ...
        with open(out_dir + '/map.csv', 'wb') as csvfile:
            ...
            writer.writerow(
                ['id', 'img_bitmap_path', 'img_np_path', 'action_np_path', 'state_np_path', 'img_bitmap_pred_path',
                 'img_np_pred_path'])
            for row in csv_ref:
                writer.writerow(row)
